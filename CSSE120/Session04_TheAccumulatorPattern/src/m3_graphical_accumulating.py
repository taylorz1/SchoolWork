...
def draw_parallel_lines(n, point, length, window):
    ...
    # ------------------------------------------------------------------
    for j in range(n):
        end = rg.Point(point.x + length, point.y + 30 * j)
        start = rg.Point(point.x, point.y + 30 * j)
...
def draw_lines(n, point, window):
    ...
    for j in range(n):
        counter = counter + 1
        end = rg.Point(x + 100, y + 100 - j * (200 / (n - 1)))
        # end2 = rg.Point(x + 100, y - 100 + j * 100 / (n / 2))
        line = rg.Line(point, end)
        # line2 = rg.Line(point, end2)
        line.attach_to(window)
