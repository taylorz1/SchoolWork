...
def test_draw_circles_from_rectangle():
    ...
    title = title + '8 blue in a row, 3 in a column; then 4 green in row, 5 in column'
    ...
    title = 'Tests 3 of DRAW_CIRLCES_FROM_RECTANGLE'
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    window.render(0.05)
    alpha = rectangle.get_center()
    r2 = rectangle.get_width() / 2
    r1 = rectangle.get_height() / 2
    for i in range(m):
        center_row = rg.Point(alpha.x - r2 - r1, alpha.y)
        circle = rg.Circle(rg.Point(center_row.x - 2 * (i * r1), center_row.y), r1)
        circle.fill_color = rectangle.fill_color
        center_column = rg.Point(alpha.x, alpha.y - r1 - r2)
        circle.attach_to(window)
        window.render(0.05)
    for i in range(n):
        circle2 = rg.Circle(rg.Point(center_column.x, center_column.y - 2 * (i * r2)), r2)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    width = rectangle1.get_width() / 2
    height = rectangle1.get_height() / 2

    window.render(0.05)
    for k in range(n):
        point1_new = rg.Point(center1.x - width * k, center1.y + height * k)
        point2_new = rg.Point(center2.x - width * k, center2.y + height * k)
        line2 = rg.Line(point1_new, point2_new)
        line2.thickness = 5
        if k & 1:  # I digress that I looked up modulus notation. There's probably another way to do this.
