...
Authors: David Mutchler, Valerie Galluzzi, Mark Hays, Amanda Stouder,
         their colleagues and Zachary Taylor.  February 2016.
...
class Box(object):
    ...
    def __init__(self, contents, volume):
        ...
        Side effects:
          ...
          -- EXCEPT if the length of the given contents
               ...
               then   self.contents is set to the emptry string
    ...
    def append_string(self, additional_contents):
        ...
        if len(self.contents + additional_contents) > self.volume:
            n = len(additional_contents) - (len(self.contents + additional_contents) - self.volume)
            self.contents = self.contents + additional_contents[0:n]
            return additional_contents[n:len(additional_contents)]
        else:
    ...
    def shrink(self, new_volume):
        ...
        if len(self.contents) > self.volume:
            n = len(self.contents) - self.volume  # How much needs to be removed
            a = self.contents[len(self.contents) - n:len(self.contents)]
            self.contents = self.contents[0:len(self.contents) - n]
            return a
        else:
            return ''
    ...
    def double_then_shrink(self, new_volume):
        ...
        a = len(self.contents)
        self.double()
        n = 2 * a - len(self.contents) + len(self.shrink(new_volume))
        return n
