...
def practice_problem3d(sequence):
    ...
    ####################################################################
    b = sequence[0]
    for n in range(int((len(sequence) + 1) / 2)):
        if b < sequence[2 * n]:
            b = sequence[2 * n]
    return b
...
def practice_problem3e(points):
    ...
    for n in range(len(a)):
        if is_prime(a[n].x) == True and is_prime(a[n].y) == True:
            b = a[n].x
            c = a[n].y
            a[n].x = c
            a[n].y = b
...
def practice_problem3f(sequence):
    ...
    alpha = 0
    for n in range(len(sequence) - 1):
        if is_prime(sequence[n]) == True:
            a = sequence[n]
            if is_prime(sequence[n + 1]) == True and a != sequence[n + 1]:
                alpha = alpha + a
    return alpha
