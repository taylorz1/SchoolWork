...
Authors: David Mutchler, Valerie Galluzzi, Mark Hays, Amanda Stouder,
         their colleagues and Zachary Taylor.  October 2015.
...
def draw_L(window, circle, r, c):
    ...
    for k in range(r):
        for j in range(3):
            center = rg.Point(circle.center.x + 2 * j * circle.radius, circle.center.y + 2 * k * circle.radius)
            circle2 = rg.Circle(center, circle.radius)
            circle2.fill_color = circle.fill_color
            circle2.attach_to(window)
            window.render(0.1)
            athing = rg.Point(circle.center.x, circle.center.y + 2 * k * circle.radius)
    for k in range(1, 4):
        for j in range(3 + c):
            center = rg.Point(athing.x + 2 * j * circle.radius, athing.y + 2 * k * circle.radius)
            circle2 = rg.Circle(center, circle.radius)
            circle2.fill_color = circle.fill_color
            circle2.attach_to(window)
            window.render(0.1)
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for k in range(n):
        for j in range(h):
            a = rg.Point(rectangle.corner_1.x - j * rectangle.get_width(), rectangle.corner_1.y + k * rectangle.get_height())
            b = rg.Point(rectangle.corner_2.x - j * rectangle.get_width(), rectangle.corner_2.y + k * rectangle.get_height())
            rect2 = rg.Rectangle(a , b)
