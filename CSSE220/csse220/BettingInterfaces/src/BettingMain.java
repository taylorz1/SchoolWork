...
public class BettingMain {
	...
	private ArrayList<BettingInterface> Bets;
	...
	private String handleRoll() {
		...
		for(BettingInterface bet : this.Bets) {
			if (bet.type() == "OddBet") {
				String bettype = "odd numbers"
			}
			if(bet.isWinResult(rollResult)) {
				System.out.println("Your bet on" + bettype + "won" + bet.winAmount());
				this.currentMoney = this.currentMoney + bet.winAmount();
			} else {
				System.out.println("Your bet on" + bettype "lost");
			}
		}
		this.Bets.clear();
	...
	private void handleHighBet(double betAmount) {
		...
		this.Bets.add(new HighRoll(betAmount));
		this.currentMoney = this.currentMoney - betAmount;
		System.out.println("ok");
	}
