...
public class LinkedList {
	...
	public void addAtEnd(int value) {
		...
		Node toadd = new Node(value,current.next);
		current.next = toadd;
		return;
	}
	...
	public void addBeforeValue(int valueToAdd, int valueToInsertBefore) {
		...
		Node toadd = new Node(valueToAdd, previous.next);
		previous.next = toadd;
	}
	...
	public LinkedList(int[] data) {
		int i = 0;
		this.head = new Node(data[i], null);
		Node current = this.head;
		for (i = 1; i < data.length; i++) {
			Node toAdd = new Node(data[i], null);
			current.next = toAdd;
			current = current.next;
	...
	public void removalAll(int nodesWithValue) {
		...
		while (current.next != null) {
			if (current.value == nodesWithValue) {
