...
public class RecursiveHelperFunctions {
	...
	public static int sumArray(int fromIndex, int toIndex, int[] array) {
		...
		int sum = 0;
		sum = array[firstIndex] + sumArray(firstIndex + 1, secondIndex, array);
		return sum;
	}
	...
	public static boolean isOrderedHelper(int index, int[] array) {
		...
		if (array[index] < array[index+1]) {
			return true && isOrderedHelper(index+1, array);
		}
		return false;
	}
	 ...
	 */
	public static int hasExactlyOneUppercaseHelper(String input, int count) {
		if (input.length() == 0) {
			return count;
		}
		if (Character.isUpperCase(input.charAt(0))) {
			count++;
			return hasExactlyOneUppercaseHelper(input.substring(1), count);
		}
		return hasExactlyOneUppercaseHelper(input.substring(1), count);
	}
	
	public static boolean hasExactlyOneUppercase(String input) {
		return hasExactlyOneUppercaseHelper(input, 0) == 1;
	}
	...
	public static int longestChainLength(String input) {
		return longestChainLengthHelper(input, 0, 0);
	}
