...
/**
 * 
 * DONE. This class implements several features of a classical gradebook and includes team functionality.
 *
 * @author taylorz1.
 *         Created Sep 15, 2016.
 */
public class TeamGradebook {
	...
	private String handleGetNames() {
		...
		for (int i = 0; i < this.students.size() - 1; i++) {
			toReturn = toReturn + this.students.get(i).getName() + " ";
		}
		toReturn = toReturn + this.students.get(this.students.size() - 1).getName();
		return toReturn;
	}
	...
	private String handleAddTeam(String teamName, ArrayList<String> memberNames) {
		...
		// DONE: Your code here
		ArrayList<Student> memberNames2 = new ArrayList<>();
		for (int i = 0; i < memberNames.size(); i++) {
			Student tempStudent = this.getStudentByName(memberNames.get(i));
			if (tempStudent == null) {
				tempStudent = new Student(memberNames.get(i));
				this.students.add(tempStudent);
			}
			memberNames2.add(tempStudent);
		}

		Team team = new Team(teamName, memberNames2);
		this.teams.add(team);
		return "ok";
	}
	...
	private String handleAddGrade(String teamName, double grade) {
		...
		for (int i = 0; i < this.teams.size(); i++) {
			if (this.teams.get(i).getName().equals(teamName)) {
				this.teams.get(i).addGrade(grade);
			}
		}
		return "ok";
	}
	...
	private String handleGetBestTeam() {
		...
		double bestTeam = 0;
		for (int i = 0; i < this.teams.size(); i++) {
			if (this.teams.get(i).getAverage() > bestTeam) {
				best = this.teams.get(i).getName();
				bestTeam = this.teams.get(i).getAverage();
			}
		}
		return best;
	}
