 ...
 *
 * @author taylorz1. Created Sep 6, 2016.
 */
public class TwelveProblems {
	...
	/**
	 * 
	 * DONE This method generates the distance of a point from the origin.
	 *
	 * @param x
	...
	/**
	 * 
	 * DONE This method determines if the second digit of an input is 5.
	 *
	 * @param input
	 * @return
	 */
	public static boolean secondDigit5(int input) {
		int value_mod = input / 10;
		if (value_mod % 10 == 0)
			return false;
		return value_mod % 5 == 0;
	}
	...
	/**
	 * 
	 * TODO This method determines if the input string ends with an uppercase
	 * letter.
	 *
	 * @param input
	 * @return
	...
	public static boolean endsWithUpperCaseLetter(String input) {
		if (input.length() > 0) {
			char str_check = input.charAt(input.length() - 1);
			return Character.isUpperCase(str_check);
		}
		if (input.isEmpty())
			return false;
		char str_check = input.charAt(0);
		return Character.isUpperCase(str_check);
	}
	...
	/**
	 * 
	 * DONE this is a power function such that inputs num, power generate
	 * num**power.
	 *
	 * @param num
	 * @param power
	...
	public static double pow(int num, int power) {
		double completed_power = 1;
		boolean negative_power = false;
		int power_run = power;
		if (power < 0) {
			power_run = -power;
			negative_power = true;
		}
		for (int count = 1; count < power_run + 1; count++) {
			completed_power = completed_power * num;
		}
		if (negative_power) {
			completed_power = 1.0 / completed_power;
		}
		return completed_power;
	}
	 ...
	 * one.charAt(0); char b = two.charAt(0); if(a == b) { System.out.println(
	...
	/**
	 * 
	 * DONE. This method returns the first index value at which two strings
	 * differ, and returns -1 if they do not differ.
	 *
	 * @param one
	 * @param two
	 * @return
	...
	/**
	 * 
	 * DONE this method determines the most common character in a string.
	 *
	 * @param input
	 * @return
	 */
	public static char mostCommonCharacter(String input) {
		    ...
		    int max_character = 0;
		    int[] character_counts = new int[200]; //I dunno how many possible characters there are but there can't be more than 200 or so.
		    for (int i = 0; i < input.length(); i++) {
		        char character_value = input.charAt(i); //I found out while fiddling that characters can be cast to integer values and therefore used as indicies.
		        if (character_counts[character_value]++ >= max_character) { //usage of character index.
		            max_character = character_counts[character_value];
		            common_character = character_value;
		        }
		    }
	...
	/**
	 * 
	 * DONE this method returns the first member of an array that is divisible by 77.
	 *
	 * @param numbers
	 * @return
	 */
	...
	/**
	 * 
	 * DONE this method creates an array (inclusive) of all the powers of 2 up to the indicated exponent.
	 *
	 * @param maxExponent
	 * @return
	 */
	public static int[] powersOfTwo(int maxExponent) {
		...
		int[] myArray = new int[maxExponent + 1];
	...
	/**
	 * 
	 * DONE this method returns the array of maximum positional values.
	 *
	 * @param one
	 * @param two
	 * @return
	...
	public static int[] maxArray(int[] one, int[] two) {
		int[] array_pair = new int[one.length];
		for (int i = 0; i < one.length; i++)
			array_pair[i] = (one[i] > two[i] ? one[i] : two[i]);
		return array_pair;
	}
	...
	/**
	 * 
	 * DONE this method counts the number of times an array sequence occurs in
	 * another longer array.
	 *
	 * @param shorter
	 *            is the array that we check for existance
	 * @param longer
	 *            is the array that we check in
	 * @return
	 */
	...
	/**
	 * 
	 * DONE this array adds a double next to each double in the string list.
	 *
	 * @param input
	 * @return
	 */
	...
	/**
	 * 
	 * DONE this returns all possible 3 character sequential strings of a given input string.
	 *
	 * @param input
	 * @return
	 */
