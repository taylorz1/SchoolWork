 ...
 *
 * @author Takahiro Yamauchi. Created September 19th, 2017.
 */
public class MapAnd2DArrayHomework {
	...
	public static char mostCommonCharacter(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			// check if given string has already been counted.
			...
			// if given char does not exist, cast key into string and then put
			// new node into map.
			if (charExists == false) {
				charCount.put(input.charAt(i), 1);
			}
		}
		// finds the character that apperared the most
		for (Character key : charCount.keySet()) {
			if (maxVal < Math.max(maxVal, charCount.get(key))) {
				maxVal = charCount.get(key);
				maxKey = key;
			}
		}
		return maxKey;
	...
	public static HashMap<String, ArrayList<Integer>> reverseHashmap(HashMap<Integer, String> initialMap) {
		HashMap<String, ArrayList<Integer>> reversedHashMap = new HashMap<String, ArrayList<Integer>>();

		for (Integer key : initialMap.keySet()) {
			Integer reversedValue = key;
			String reversedKey = initialMap.get(key);
			boolean strExists = false;

			// if reversedHashMap contains the key, add the key.
			if (reversedHashMap.containsKey(reversedKey)) {
				ArrayList<Integer> tempArr = reversedHashMap.get(reversedKey);
				tempArr.add(reversedValue);
				reversedHashMap.put(initialMap.get(key), tempArr);
				strExists = true;
			}
			if (strExists == false) {
				...
				reversedHashMap.put(reversedKey, tempArr);
			}
		}
	...
	public static String getTemperatureDropCity(int[] temps, String[] tempCity) {
		HashMap<String, ArrayList<Integer>> cityTemp = new HashMap<String, ArrayList<Integer>>();

		for (int i = 0; i < tempCity.length; i++) {
			String key = tempCity[i];
			int value = temps[i];

			if (cityTemp.containsKey(key)) {
				ArrayList<Integer> temp = cityTemp.get(key);
				temp.add(value);
				cityTemp.put(key, temp);
			} else {
	...
	public static int getNumberOfCoursesToTake(HashMap<String, String> courseMap, String course) {
		if (courseMap.containsKey(course) == false)
			return -1;

		String key = course;
		String val = "";
		int prereqCount = 0;
		int infiniteLoop = courseMap.size() * 2;
		...
		while (true) {
			...
			// if looping insanely, consider it as an infinite loop and return
			// -1
			if (infiniteLoop <= 0)
				return -1;
			infiniteLoop--;
	...
	public static int longestRepeatSequence(char[][] data) {
		...
		for (int i = 0; i < data.length; i++) {
			for (int j = 0; j < data[0].length; j++) {
				if (data[i][j] != charLookedAt) {
					charLookedAt = data[i][j];
					charCount = 1;
				} else if (data[i][j] == charLookedAt) {
					charCount += 1;
					frequentCharCount = Math.max(frequentCharCount, charCount);
				}
			}
		}
		return frequentCharCount;
	}
	...
	public static String stringFromColumns(char[][] data) {
		...
		for (int j = 0; j < data[0].length; j++) {
			for (int i = 0; i < data.length; i++) {
				joined += String.valueOf(data[i][j]);
			}
		}
	...
	public static char[][] distanceArray(int row, int col, int distance) {
		...
		map[row][col] = 'x'; // the given position must be x
		return map;
	}
	...
	public static int ballRestElevation(int[][] map) {
		...
		int y = 1;
		int min = 10;
		char dropDir = ' '; // the direction where the ball would drop. u=up,
							// d=down,
							// l=left, and r=right
		...
		while (true) {
			...
			// check down
			if (x + 1 < map.length && min > map[x + 1][y]) {
				min = map[x + 1][y];
				dropDir = 'd';
				changed = true;
			}
			...
			// check right
			if (y + 1 < map[0].length && min > map[x][y + 1]) {
				min = map[x][y + 1];
				dropDir = 'r';
				changed = true;
			}
			...
			// adjusting cordinates depending on the direction of the ball
			// fall(dropDir)
			switch (dropDir) {
			case 'u':
				x--;
