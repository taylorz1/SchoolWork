...
public class TwelveProblems {
	...
	public static boolean secondDigit5(int input) {
		...
		int whereat = inputstring.length();
		if(whereat <= 1) {
			return (false);
		}
		if(inputstring.charAt(whereat-2) == '5') {
			current = true;
		}
		else{
			current = false;
	...
	public static boolean endsWithUpperCaseLetter(String input) {
		boolean current = true;
		int whereat = input.length();
		if(whereat <= 1) {
			return (false);
		}
		if(Character.isUpperCase(input.charAt(whereat-1))) {
			current = true;
		}
		else{
			current = false;
	...
	public static int timesOccur(int[] shorter, int[] longer) {
		...
		for(int i=0;i<(longer.length-1);i++){
			for(int j=0;j<shorter.length;j++) {
				list1 = list1 + shorter[j];
				list2 = list2 + longer[i+j];
				if((i+j)>longer.length-2) {
					break;
				}
			}
