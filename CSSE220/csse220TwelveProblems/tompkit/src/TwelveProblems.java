...
//			}
	...
	public static char mostCommonCharacter(String input) 
	{
		...
		for(int index=0;index<input.length();index++)
		{
			for(int times=0;times<input.length();times++)
				if(input.charAt(index)==input.charAt(times))
					currentChar++;
			if(max<currentChar)
			{
				max=currentChar;
	...
	public static ArrayList<String> threeCharacterStrings(String input) 
	{
		...
		if(input.length()>2)
			list.add(input.substring(0,3));
		if(input.length()>3)
			list.add(input.substring(1,4));
		if(input.length()>4)
			list.add(input.substring(2,5));
		return list;
	}
