...
public class TwelveProblems {
	...
	public static int[] powersOfTwo(int maxExponent) {
		if (maxExponent>=0) { 
			int[] twoP=new int[maxExponent+1];
			int k=0;
			for (int i : twoP) {
	...
	public static int timesOccur(int[] shorter, int[] longer) {
		int sum=0;
		int[] new1=new int[shorter.length];
		 
		for (int k=0;k<=longer.length-shorter.length;k++) {
			for (int i=0;i<shorter.length;i++) { 	
				new1[i]=longer[k+i]; 
				}
				
			if (Arrays.equals(shorter,new1)) {
					sum++;  
			}
		}
		return sum;
	...
	public static ArrayList<String> threeCharacterStrings(String input) {
		...
		for (int i =0; i<input.length()-2;i++) {
			String sub=String.valueOf(input.charAt(i))+String.valueOf(input.charAt(i+1))+String.valueOf(input.charAt(i+2));
			sub3.add(sub);
		}	
		return sub3;
	}
