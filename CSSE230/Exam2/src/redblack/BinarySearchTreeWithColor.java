...
//		if (this.root.Black()) { //Ruins other test cases as it doesn't work.
//			return false;
//		}
		return true;
	}
	...
	public class RedBlackNode {
		...
		public boolean Black() {
			//Need to find the number of blacks in each branch and then
			//somehow compare each branch to each other branch.
			//For some reason I just can't think of how to compare each branch to it's
			//other branch.
			int one = this.left.sumOfBlack();
			int two = this.right.sumOfBlack();
			return false;
		}
		...
		public int sumOfBlack() {
			...
			if (this.left != NULL_NODE && this.right != NULL_NODE) {
				if (this.color == Color.RED) {
					return this.left.sumOfBlack() + this.right.sumOfBlack();
				} 
				return this.left.sumOfBlack() + this.right.sumOfBlack() + 1;
				...
				if (this.color == Color.RED) {
					return this.right.sumOfBlack();
				}
				return this.right.sumOfBlack() + 1;
			}
			return 0;
		}
