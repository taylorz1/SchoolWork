 ...
 * 
 * @author Matt Boutell and Zach Taylor
 */
public class TwoDTree {
	...
	public void insert(Point2 p, String label) {
		...
		if (this.root == NULL_NODE) {
			this.root = new Node(p.x, p.y, label, Direction.X, new RectHV(0, 0, 1, 1), 0);
		} else {
			this.root = this.root.insert(p, label, this.root, false, 0); 
		}
	}
	...
	public Point2 nearestNeighbor(Point2 q) throws IllegalStateException {
		// DONE: write this.
		Wrapper topass = new Wrapper(q, this.root.p.distanceTo(q), this.root.p);
		this.root.bottomRight.nearestNeighbor(topass);
		this.root.topLeft.nearestNeighbor(topass);
		
		return topass.pointer;
	...
	public class Wrapper {
		...
		public Wrapper (Point2 toTest, double distance, Point2 pointer) { 
			this.toTest = toTest;
	...
	public class Node {
		...
		public void nearestNeighbor(Wrapper topass) {
			// DONE Auto-generated method stub.
			if (this == NULL_NODE) {
				return;
			}
			if (this.bounds.distanceTo(topass.toTest) > topass.distance) {
				return;
			}
			
			double distance = topass.toTest.distanceTo(this.p);
			if (distance < topass.distance) {
				topass.distance = distance;
				topass.pointer = this.p;
				
			}
			
			this.topLeft.nearestNeighbor(topass);
			this.bottomRight.nearestNeighbor(topass);
		...
		public Node insert(Point2 p2, String label2, Node parent, boolean left, int depth) {
			// TODO Auto-generated method stub.
			boolean xdir = parent.dir == Direction.X;
			if (this == NULL_NODE) {
				RectHV bounds = parent.bounds;
				RectHV creation;
				if (xdir) {
					if (left) {
						creation = new RectHV(bounds.xmin(), bounds.ymin(), parent.p.x, bounds.ymax());
						return new Node(p2.x, p2.y, label2, Direction.Y, creation, depth);
					}
					creation = new RectHV(parent.p.x, bounds.ymin(), bounds.xmax(), bounds.ymax());
					return new Node(p2.x, p2.y, label2, Direction.Y, creation, depth);
				}
				if (left) {
					creation = new RectHV(bounds.xmin(), bounds.ymin(), bounds.xmax(), parent.p.y);
					return new Node(p2.x,p2.y,label2,Direction.X, creation, depth);
				}
				creation = new RectHV(bounds.xmin(), parent.p.y, bounds.xmax(), bounds.ymax());
				return new Node(p2.x,p2.y,label2,Direction.X, creation, depth);
			...
			if (this.dir == Direction.X) {
				if (this.p.x > p2.x) {
					this.topLeft = this.topLeft.insert(p2, label2, this, true, depth+1);
				} else {
					this.bottomRight = this.bottomRight.insert(p2, label2, this, false, depth+1);
				}
				return this;
			}
			if (this.p.y > p2.y) {
