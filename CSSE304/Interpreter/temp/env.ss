        ...
        (cond [(list? syms)
              ...
              [(symbol? syms)
                (if (eqv? sym syms)
                  ...
                  (apply-env env sym succeed fail))]
              [else
                (letrec ([iter (lambda (ls n)
                              (cond [(null? ls) (list '())]
                                [(zero? n) (list ls)]
                                [else
                                  (cons (car ls) (iter (cdr ls) (- n 1)))]))]
                      [newls (make-list syms)])
                  (let ([pos (list-find-position sym newls)]
                        [newvals (iter vals (- (length vals) (length newls)))])
                      (if   (number? pos)
                    (succeed (list-ref newvals pos))
                    (apply-env env sym succeed fail))))])
])))
