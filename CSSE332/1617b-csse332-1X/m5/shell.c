...
int main() {
  ...
  while(1) {
    ...
    } else if (stringcmp(line, "kill \0") == 0) {
      interrupt(0x21, 9, line[5]-48, 0, 0);
    } else if (stringcmp(line, "execforeground \0") == 0) {
      interrupt(0x21, 10, line + 15, 0, 0);
    } else if (stringcmp(line, "shellcolor \0") == 0) {
      interrupt(0x21, -4, line[11]-48, 0, 0);
    } else if (stringcmp(line, "cls\0") == 0) {
      interrupt(0x21, -3, 0, 0, 0);
    } else if (stringcmp(line, "help\0") == 0) {
      interrupt(0x21, 0, "type - Prints contents of the file\r\nexecute - Runs the program\r\ncopy - Copies the file\r\ndelete - Deletes the file\r\ndir - Prints contents of the directory\r\nshutdown - Shuts the OS down\r\ncreate - Creates a new file\r\nkill - Kills process at index\r\nexecforeground - Runs program without interruption\r\nshellcolor - Changes color of background\r\ncls - Clear screen\r\n\0", 0, 0);
    }
    else {
      interrupt(0x21,0,"Not a valid command, type help for list of commands\r\n\0",0,0);
    }
  }
}
