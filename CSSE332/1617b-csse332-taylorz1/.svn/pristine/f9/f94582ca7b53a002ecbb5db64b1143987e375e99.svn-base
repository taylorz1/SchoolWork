...
void last4(int* start, int* end, int** outputPosition) {
  // there is a nicer (trickier) solution where we count backwards but
  // this is the straightfoward approach
  int* last4 = NULL;
  while(start != end) {
    if(*start == 4) {
      last4 = start;
...
int linkedListEqualAndDistinct(struct LinkedListNode* list1,
  ...
  // this would be most elegantly solved with recursion
  // but some people find that weird so I've done it iteratively

  while(list1 != NULL && list2 != NULL) {
    if(list1 == list2) {
      //the lists will forever more be equal but they are not distinct
      return 2;
    }
    if((*list1).data != (*list2).data) {
      return 0; //lists differ
    }
    list1 = (*list1).next;
    list2 = (*list2).next;
  }
