...
public class DecoratorDetector extends PatternDetectAnalyzor {
	...
	protected void detect() {
		...
		for (RelationObject r : rel) {
			if (decorates(r)) {
				Pattern pattern = new Pattern("decorator");
				...
				pattern.addComponent("decorator", from);
				for (SootClass c : model.getSootClass()) {
					if (c.getSuperclass().equals(from)) {
						pattern.addComponent("decorator", c);
					}
				}
	...
	private boolean decorates(RelationObject r) {
		...
		for (RelationObject ro : relation) {
			...
			if (ro.getType().equals(Enums.Enum.ASSOCIATION) && ro.getFrom().equals(r.getFrom())) {
				if (findSuperclass(ro.getTo(), r.getTo())) {
	...
	private boolean checkMethod(SootClass from, SootClass to) {
		...
		if(methodsInSub.size() != methodsInSuper.size()){
			return false;
		}else{
	...
	// private boolean decoratorHelper(SootField f, SootClass to){
	// List<RelationObject> associations =
	// model.getRelations().stream().filter(rm ->
	// rm.getType().equals(Enums.Enum.ASSOCIATION)).collect(Collectors.toList());
	// for(RelationObject r : )
	// return false;
	// }

	private boolean containsMethod(List<SootMethod> sm, SootMethod m) {
		for (SootMethod method : sm) {
