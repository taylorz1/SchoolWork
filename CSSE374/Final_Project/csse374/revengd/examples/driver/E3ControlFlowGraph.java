...
public class E3ControlFlowGraph implements Runnable {
	...
	public void run() {
		...
		SootClass calculatorApp = scene.getSootClass("csse374.revengd.examples.fixtures.TestClass");
		SootMethod mainMethod = calculatorApp.getMethodByName("add");
		...
		prettyPrint("Statements in the TestClass.add() method", cfg);		// Prints the statements in the control flow graph
	...
	void prettyPrint(String title, Iterable<Unit> stmts) {
		...
		stmts.forEach(stmt -> {
			if (stmt instanceof AssignStmt) {
				...
				if(rightOp instanceof InvokeExpr){
				System.out.println(((InvokeExpr)rightOp).getMethod().getDeclaringClass().getName());
				System.out.println(((InvokeExpr)rightOp).getMethod().getName());
